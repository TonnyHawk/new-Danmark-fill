   ...
   function Nav(id){
      ...
      this.adaptize = () => {
         ...
         if(defineBreakpoint() != null){// if we have a breakpoint
            ...
            let areVisible = numOfVisible(defineBreakpoint());// how much items should be visible

            if(areVisible != null){
               this.createExpand();

               let allItems = this.getNumOfItem('menu');
               let diff = allItems - areVisible;// how much items should be replaced to the expand tab
               for(i = 0; i < diff; i++){
